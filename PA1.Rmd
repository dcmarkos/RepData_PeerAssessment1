...
```
**For total steps taken per day, the mean is `r format(mean[1],scientific=FALSE)` and the median is `r format(median[1],scientific=FALSE)`. Note: these values were obtained by removing na entries.**

*With na entries converted to 0, mean = `r format(meanNA[1],scientific=FALSE)` and median = `r format(medianNA[1],scientific=FALSE)`. This is a misleading result.*
...
```
**With imputed values for total steps taken per day, the mean is `r format(mean2[1],scientific=FALSE)` and the median is `r format(median2[1],scientific=FALSE)`.**

**The before-after difference for the mean is  `r mean[1] - mean2[1]`, and for the median it is `r median[1] - median2[1]`.**
